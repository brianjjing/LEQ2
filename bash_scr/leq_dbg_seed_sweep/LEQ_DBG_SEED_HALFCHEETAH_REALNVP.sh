#!/bin/bash
set -e
# LEQ + DBG (RealNVP) with a FIXED reward_penalty_coef, across SEEDS 42 123 456,
...
#   2. Verify a PRE-TRAINED RealNVP guardian exists per seed at
#      GUARDIAN_ROOT/realnvp_<seed> (this script does NOT train guardians).
#   3. Run LEQ once per seed in SEEDS, ALL IN PARALLEL, one job per GPU,
#      fixed reward_penalty_coef (from LAMBDA_TABLE["halfcheetah-m-e-sparse"]["RealNVP"]).
#
# GPU layout (default): GPUs 1-3.
#
# Usage (from LEQ2 root):
#   bash bash_scr/leq_dbg_seed_sweep/LEQ_DBG_SEED_HALFCHEETAH_REALNVP.sh
#
# Env overrides:
...
# Fixed reward_penalty_coef -- LAMBDA_TABLE["halfcheetah-m-e-sparse"]["RealNVP"].
COEF="${COEF:-0.8}"
...
echo "============================================"
echo "LEQ + DBG (RealNVP) seed sweep on sparse HalfCheetah"
echo "  Env task:      $TASK"
...
echo "Step 2/3: Verify pre-trained RealNVP guardians (one per seed)"
declare -A GUARDIAN_FOR_SEED
for seed in "${SEEDS[@]}"; do
    p="$GUARDIAN_ROOT/realnvp_${seed}"
    if [ -f "${p}_model.pth" ] && [ -f "${p}_meta_data.pkl" ]; then
        echo "  seed=$seed -> $p (found)"
        GUARDIAN_FOR_SEED[$seed]="$p"
    else
        echo "ERROR: pre-trained RealNVP guardian not found at $p"
