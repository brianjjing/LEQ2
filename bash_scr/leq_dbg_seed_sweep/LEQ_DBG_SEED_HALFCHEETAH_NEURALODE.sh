#!/bin/bash
set -e
# LEQ + DBG (NeuralODE) with a FIXED reward_penalty_coef, across SEEDS 42 123 456,
# on sparse HalfCheetah (72.5% sparse).
#
# Uses the sparse offline pickle for LEQ:
#   /public/d4rl/sparse_datasets/halfcheetah_medium_expert_sparse_72.5.pkl
#
# Pipeline:
...
#   2. Verify a PRE-TRAINED NeuralODE guardian exists at GUARDIAN_ROOT/neuralODE
#      (this script does NOT train the guardian). NOTE: unlike the other
#      guardian types, there is only ONE NeuralODE checkpoint per env (no
#      per-seed diffusion_<seed>/vae_<seed>-style variants exist on disk),
#      so the SAME guardian is reused for every seed below.
#   3. Run LEQ once per seed in SEEDS, ALL IN PARALLEL, one job per GPU,
#      fixed reward_penalty_coef (from LAMBDA_TABLE["halfcheetah-m-e-sparse"]["NeuralODE"]).
#
# GPU layout (default): GPUs 1-3.
#
# Usage (from LEQ2 root):
#   bash bash_scr/leq_dbg_seed_sweep/LEQ_DBG_SEED_HALFCHEETAH_NEURALODE.sh
#
# Env overrides:
...
DYN_TAG="${DYN_TAG:-halfcheetah-medium-expert-v2_sparse_72.5}"
GUARDIAN_ROOT="${GUARDIAN_ROOT:-/public/gormpo/models/halfcheetah_medium_expert_sparse_3}"
DBG="neuralode"

# Fixed reward_penalty_coef -- LAMBDA_TABLE["halfcheetah-m-e-sparse"]["NeuralODE"].
COEF="${COEF:-0.1}"
...
echo "============================================"
echo "LEQ + DBG (NeuralODE) seed sweep on sparse HalfCheetah"
echo "  Env task:      $TASK"
echo "  Dataset:       $DATASET_PATH"
echo "  Dynamics tag:  $DYN_TAG"
echo "  Dynamics seed: $DYN_SEED (fixed, reused for all LEQ seeds)"
echo "  Guardian:      $GUARDIAN_PATH (single checkpoint, reused for all seeds)"
...
if [ -f "${p}/model.pt" ] && [ -f "${p}/metadata.pkl" ]; then
    echo "  Found guardian. Not training -- reusing as-is for every seed."
else
