#!/bin/bash
set -e
# Train LEQ on sparse HalfCheetah-Medium-Expert with the expectile chosen by
# the expectile hyperparameter search (bash_scr/expectile_search), WITHOUT a
...
#
# GPU layout (fixed, dedicated -- does not overlap with the hopper/walker2d
# scripts, so this can run alongside either of them):
#   GPUs 4-5, one per seed.
#
# Usage (from LEQ2 root):
#   bash bash_scr/leq_sparse_train/LEQ_SPARSE_HALFCHEETAH.sh
#
# Env overrides:
...
# Expectile chosen by the expectile search for HalfCheetah.
EXPECTILE="${EXPECTILE:-0.1}"
...
echo "============================================"
echo "LEQ training on sparse HalfCheetah (no guardian)"
echo "  Env task:      $TASK"
