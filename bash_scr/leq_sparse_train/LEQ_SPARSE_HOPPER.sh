#!/bin/bash
set -e
# Train LEQ on sparse Hopper-Medium-Expert with the expectile chosen by the
...
#   GPUs 0-1, one per seed. NOTE: LEQ_SPARSE_WALKER2D.sh uses GPUs 2-3 and
#   LEQ_SPARSE_HALFCHEETAH.sh uses GPUs 4-5, so all three can run together.
#
# Usage (from LEQ2 root):
#   bash bash_scr/leq_sparse_train/LEQ_SPARSE_HOPPER.sh
#
# Env overrides:
...
# Expectile chosen by the expectile search for Hopper.
EXPECTILE="${EXPECTILE:-0.3}"
...
echo "============================================"
echo "LEQ training on sparse Hopper (no guardian)"
echo "  Env task:      $TASK"
