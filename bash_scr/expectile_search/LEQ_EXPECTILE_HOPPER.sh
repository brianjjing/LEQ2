#!/bin/bash
set -e
# Expectile hyperparameter search for LEQ on sparse Hopper-Medium-Expert,
...
#   GPUs 0-3, one per expectile value. NOTE: LEQ_EXPECTILE_WALKER2D.sh also
#   uses GPUs 0-3 -- do not run both scripts at the same time unless you
#   override GPUS below.
#
# Usage (from LEQ2 root):
#   bash bash_scr/expectile_search/LEQ_EXPECTILE_HOPPER.sh
#
# Env overrides:
...
echo "============================================"
echo "LEQ expectile search on sparse Hopper (no guardian)"
echo "  Env task:      $TASK"
