#!/bin/bash
set -e
# reward_penalty_coef hyperparameter search for LEQ + DBG (VAE) on
...
#   2. Verify a PRE-TRAINED VAE guardian already exists at
...
#   bash bash_scr/leq_dbg_penalty_search/LEQ_DBG_SEARCH_HALFCHEETAH_VAE.sh
#
# Env overrides:
...
GUARDIAN_PATH="$GUARDIAN_ROOT/vae_${SEED}"
...
echo "============================================"
echo "LEQ + DBG (VAE) reward_penalty_coef search on sparse HalfCheetah"
echo "  Env task:      $TASK"
...
echo "Step 2/3: Verify pre-trained VAE guardian -> $GUARDIAN_PATH"
p="$GUARDIAN_PATH"
if [ -f "${p}_model.pth" ] && [ -f "${p}_meta_data.pkl" ]; then
    echo "  Found guardian. Not training -- reusing as-is."
else
    echo "ERROR: pre-trained VAE guardian not found at $GUARDIAN_PATH"
