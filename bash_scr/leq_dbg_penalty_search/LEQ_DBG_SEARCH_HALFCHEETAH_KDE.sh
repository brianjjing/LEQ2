#!/bin/bash
set -e
# reward_penalty_coef hyperparameter search for LEQ + DBG (KDE) on
...
#   2. Verify a PRE-TRAINED KDE guardian already exists at
...
#   bash bash_scr/leq_dbg_penalty_search/LEQ_DBG_SEARCH_HALFCHEETAH_KDE.sh
#
# Env overrides:
...
echo "============================================"
echo "LEQ + DBG (KDE) reward_penalty_coef search on sparse HalfCheetah"
echo "  Env task:      $TASK"
...
echo "Step 2/3: Verify pre-trained KDE guardian -> $GUARDIAN_PATH"
p="$GUARDIAN_PATH"
if [ -f "${p}.faiss" ] && [ -f "${p}_metadata.pkl" ]; then
    echo "  Found guardian. Not training -- reusing as-is."
else
    echo "ERROR: pre-trained KDE guardian not found at $GUARDIAN_PATH"
