#!/bin/bash
set -e
# reward_penalty_coef hyperparameter search for LEQ + DBG (DDPM) on
# sparse HalfCheetah (72.5% sparse), SEED 42 ONLY.
#
...
#   2. Verify a PRE-TRAINED DDPM guardian already exists at
...
#
# GPU layout (fixed): GPUs 4-7 -- does not overlap with the hopper/walker2d search scripts below.
#
# Usage (from LEQ2 root):
#   bash bash_scr/leq_dbg_penalty_search/LEQ_DBG_SEARCH_HALFCHEETAH_DDPM.sh
#
# Env overrides:
...
GUARDIAN_PATH="$GUARDIAN_ROOT/diffusion_${SEED}"
...
echo "============================================"
echo "LEQ + DBG (DDPM) reward_penalty_coef search on sparse HalfCheetah"
echo "  Env task:      $TASK"
...
echo "Step 2/3: Verify pre-trained DDPM guardian -> $GUARDIAN_PATH"
p="$GUARDIAN_PATH"
if [ -f "${p}/checkpoint.pt" ]; then
    echo "  Found guardian. Not training -- reusing as-is."
else
    echo "ERROR: pre-trained DDPM guardian not found at $GUARDIAN_PATH"
