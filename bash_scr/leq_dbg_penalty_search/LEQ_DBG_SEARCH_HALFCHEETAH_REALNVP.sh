#!/bin/bash
set -e
# reward_penalty_coef hyperparameter search for LEQ + DBG (RealNVP) on
...
#   2. Verify a PRE-TRAINED RealNVP guardian already exists at
...
#   bash bash_scr/leq_dbg_penalty_search/LEQ_DBG_SEARCH_HALFCHEETAH_REALNVP.sh
#
# Env overrides:
...
GUARDIAN_PATH="$GUARDIAN_ROOT/realnvp_${SEED}"
...
echo "============================================"
echo "LEQ + DBG (RealNVP) reward_penalty_coef search on sparse HalfCheetah"
echo "  Env task:      $TASK"
...
echo "Step 2/3: Verify pre-trained RealNVP guardian -> $GUARDIAN_PATH"
p="$GUARDIAN_PATH"
if [ -f "${p}_model.pth" ] && [ -f "${p}_meta_data.pkl" ]; then
    echo "  Found guardian. Not training -- reusing as-is."
else
    echo "ERROR: pre-trained RealNVP guardian not found at $GUARDIAN_PATH"
