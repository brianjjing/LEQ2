...
# LEQ + DBG (DDPM) on sparse Walker2d (73% sparse), 3 seeds launched IN PARALLEL
...
#   bash bash_scr/leq_dbg_new/LEQ_DBG_WALKER2D_DDPM.sh
#   GPU_IDS="0 1 2" bash bash_scr/leq_dbg_new/LEQ_DBG_WALKER2D_DDPM.sh
...
if [ -n "${SEED:-}" ]; then
    ...
    if [ "${DETACH:-1}" = 1 ] && [ -z "${LEQ_DETACHED:-}" ]; then
        ...
        LOGFILE="$LEQ2_DIR/tmp/_runs/WALKER2D_ddpm_seed${SEED}_$(date +%m%d-%H%M%S).log"
