...
# LEQ + DBG (REALNVP) on sparse Hopper (78% sparse), 3 seeds launched IN PARALLEL
# (one self-detaching background job per seed, one GPU each).
#
# Split out of bash_scr/leq_dbg/LEQ_DBG_HOPPER.sh, which loops over all 5
...
# estimator) pair -- same number as bash_scr/leq_dbg/LEQ_DBG_HOPPER.sh's
# REWARD_PENALTY_COEF[realnvp], sourced from
# GORMPO/configs/realnvp/gormpo_hopper_medium_expert_sparse_3.yaml.
#
...
#   bash bash_scr/leq_dbg_new/LEQ_DBG_HOPPER_REALNVP.sh
#   GPU_IDS="0 1 2" bash bash_scr/leq_dbg_new/LEQ_DBG_HOPPER_REALNVP.sh
#   DETACH=0 bash bash_scr/leq_dbg_new/LEQ_DBG_HOPPER_REALNVP.sh   # foreground, all 3 seeds interleaved
#
# Env overrides:
...
DYN_TAG="hopper-medium-expert-v2_sparse_78"
CONFIG_TAG="hopper_medium_expert_sparse_3"
...
run_seed() {
    ...
    # Resume: skip entirely if LEQ already finished (same check as LEQ_DBG_HOPPER.sh).
...
if [ -n "${SEED:-}" ]; then
    ...
    if [ "${DETACH:-1}" = 1 ] && [ -z "${LEQ_DETACHED:-}" ]; then
        ...
        LOGFILE="$LEQ2_DIR/tmp/_runs/HOPPER_realnvp_seed${SEED}_$(date +%m%d-%H%M%S).log"
