...
# LEQ + DBG (KDE) on sparse HalfCheetah (72.5% sparse), 3 seeds launched IN PARALLEL
# (one self-detaching background job per seed, one GPU each).
#
# Split out of bash_scr/leq_dbg/LEQ_DBG_HALFCHEETAH.sh, which loops over all 5
...
# estimator) pair -- same number as bash_scr/leq_dbg/LEQ_DBG_HALFCHEETAH.sh's
# REWARD_PENALTY_COEF[kde], sourced from
# GORMPO/configs/kde/gormpo_halfcheetah_medium_expert_sparse_3.yaml.
#
...
#   bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_KDE.sh
#   GPU_IDS="0 1 2" bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_KDE.sh
#   DETACH=0 bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_KDE.sh   # foreground, all 3 seeds interleaved
#
# Env overrides:
...
DIFFUSION_NPZ="${DIFFUSION_NPZ:-/public/d4rl/sparse_datasets/diffusion_processed/halfcheetah_medium_expert_sparse_72.5_train.npz}"
GUARDIAN_ROOT="${GUARDIAN_ROOT:-/public/gormpo/models/halfcheetah_medium_expert_sparse_3}"
DYN_TAG="halfcheetah-medium-expert-v2_sparse_72.5"
CONFIG_TAG="halfcheetah_medium_expert_sparse_3"
REWARD_PENALTY_COEF="${REWARD_PENALTY_COEF:-0.1}"  # GORMPO's tuned value, see header
...
guardian_path() {
    local seed="$1"
    echo "$GUARDIAN_ROOT/kde_${seed}"
}

# --- 0 (true) if the guardian at path $1 already exists ---
guardian_exists() {
    local path="$1"
    [ -f "${path}.faiss" ] && [ -f "${path}_metadata.pkl" ]
}
...
train_guardian() {
    ...
    (cd "$GORMPO_ROOT" && python kde_module/kde.py \
        ...
        --seed "$seed" --save_path "$path" --devid 0)
}
...
run_seed() {
    ...
    # Resume: skip entirely if LEQ already finished (same check as LEQ_DBG_HALFCHEETAH.sh).
...
if [ -n "${SEED:-}" ]; then
    ...
    if [ "${DETACH:-1}" = 1 ] && [ -z "${LEQ_DETACHED:-}" ]; then
        ...
        LOGFILE="$LEQ2_DIR/tmp/_runs/HALFCHEETAH_kde_seed${SEED}_$(date +%m%d-%H%M%S).log"
