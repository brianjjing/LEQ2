...
# LEQ + DBG (NEURALODE) on sparse HalfCheetah (72.5% sparse), 3 seeds launched IN PARALLEL
...
# GORMPO/configs/neuralODE/gormpo_halfcheetah_medium_expert_sparse_3.yaml.
#
...
#   bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_NEURALODE.sh
#   GPU_IDS="0 1 2" bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_NEURALODE.sh
#   DETACH=0 bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_NEURALODE.sh   # foreground, all 3 seeds interleaved
#
# Env overrides:
...
if [ -n "${SEED:-}" ]; then
    ...
    if [ "${DETACH:-1}" = 1 ] && [ -z "${LEQ_DETACHED:-}" ]; then
        ...
        LOGFILE="$LEQ2_DIR/tmp/_runs/HALFCHEETAH_neuralode_seed${SEED}_$(date +%m%d-%H%M%S).log"
