...
# LEQ + DBG (REALNVP) on sparse HalfCheetah (72.5% sparse), 3 seeds launched IN PARALLEL
...
# REWARD_PENALTY_COEF[realnvp], sourced from
# GORMPO/configs/realnvp/gormpo_halfcheetah_medium_expert_sparse_3.yaml.
#
...
#   bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_REALNVP.sh
#   GPU_IDS="0 1 2" bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_REALNVP.sh
#   DETACH=0 bash bash_scr/leq_dbg_new/LEQ_DBG_HALFCHEETAH_REALNVP.sh   # foreground, all 3 seeds interleaved
#
# Env overrides:
...
REWARD_PENALTY_COEF="${REWARD_PENALTY_COEF:-0.8}"  # GORMPO's tuned value, see header
...
guardian_path() {
    local seed="$1"
    echo "$GUARDIAN_ROOT/realnvp_${seed}"
}
...
if [ -n "${SEED:-}" ]; then
    ...
    if [ "${DETACH:-1}" = 1 ] && [ -z "${LEQ_DETACHED:-}" ]; then
        ...
        LOGFILE="$LEQ2_DIR/tmp/_runs/HALFCHEETAH_realnvp_seed${SEED}_$(date +%m%d-%H%M%S).log"
