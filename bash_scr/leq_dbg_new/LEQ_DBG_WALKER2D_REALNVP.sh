...
# LEQ + DBG (REALNVP) on sparse Walker2d (73% sparse), 3 seeds launched IN PARALLEL
...
# GORMPO/configs/realnvp/gormpo_walker2d_medium_expert_sparse_3.yaml.
#
...
#   bash bash_scr/leq_dbg_new/LEQ_DBG_WALKER2D_REALNVP.sh
#   GPU_IDS="0 1 2" bash bash_scr/leq_dbg_new/LEQ_DBG_WALKER2D_REALNVP.sh
#   DETACH=0 bash bash_scr/leq_dbg_new/LEQ_DBG_WALKER2D_REALNVP.sh   # foreground, all 3 seeds interleaved
#
# Env overrides:
...
if [ -n "${SEED:-}" ]; then
    ...
    if [ "${DETACH:-1}" = 1 ] && [ -z "${LEQ_DETACHED:-}" ]; then
        ...
        LOGFILE="$LEQ2_DIR/tmp/_runs/WALKER2D_realnvp_seed${SEED}_$(date +%m%d-%H%M%S).log"
